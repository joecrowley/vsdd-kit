# VSDD Setup Reference

The companion to [`SETUP.md`](../SETUP.md). You don't need it for a normal install:
the installer does Steps 0–5, and `SETUP.md` covers everything left to do. Read the
part of this file you need when:

| Situation | Read |
|---|---|
| The installer stopped for a decision (exit 3) and you need the detail to explain the options | The step it names, below |
| The installer failed (exit 1), or can't be used | The failed step onwards, below: each step is the manual version, with its own **Verify** |
| The installer left a Step 2–5 item it couldn't do safely | That step, below |
| The project opens an editor workspace with a shared `/opsx` folder, or you want the kit's tooling outside the project | [Workspaces with a shared command folder](#workspaces-with-a-shared-command-folder) |
| Later: `openspec update`, a kit or OpenSpec upgrade, a new AI tool | [Maintenance](#maintenance-tell-the-user) |
| Something doesn't work | [Troubleshooting](#troubleshooting) |
| Undoing an install, or removing VSDD for good | [Roll back an install](#roll-back-an-install), [Uninstall](#uninstall) |

`KIT`, `ROOT` and `TOOLS` mean the same as in [`SETUP.md`](../SETUP.md#variables),
and its rules for executing the runbook apply here too.
...
- **`config.yaml` exists:**
  ...
  6. Add `context:` if it's missing, filled in as [`SETUP.md`](../SETUP.md#write-the-project-context) describes.

**Verify** with a throwaway change (`SETUP.md` Step 8 uses it, then removes it):
